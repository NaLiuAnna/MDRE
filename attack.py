# Classes for character-level(typo), word-level(synonym replacement), and phrase-level(SEAs) textual
# adversarial attacks
import os
import random
import string
...
class Typo:
    ...
    3) add: internally inserting a new character in a randomly chosen word.
    4) Keyboard mistakes: substituting an internal character with one of its adjacent characters in keyboards.
    """

    def __init__(self, predict, max_len, num_labels):
        """
        Create a Typo instance.
        :param predict: a function used for prediction, the purpose of the attack is to change predictions of
                        original inputs.
        :param max_len: The maximum sequence length.
        :param num_labels: the number of classes in the dataset. 2 for IMDB, 3 for Mnli.
        """
    ...
    def generate(self, texts, text_pairs, labels):
        """
        Generate character-level(Typo) adversarial examples
        :param texts, text_pairs, labels: original texts, text_pairs, labels
        :return: original, adversarial examples, their labels and predictions
        """
        # maximum number of typo attacks on original examples
        n_try = self.max_length // 2

        # get original examples' predictions
        preds, _ = self.predict(list(zip(texts, text_pairs)))
        ...
        for i, (sent1, sent2, pred) in enumerate(zip(texts, text_pairs, preds)):
            ...
            if corr_pred[i]:
                ...
                if adv_ind.size != 0:
                    adv_texts.append(i_advs[adv_ind[0]][0])
                    adv_text_pairs.append(i_advs[adv_ind[0]][1]) if i_advs[adv_ind[0]][1] else adv_text_pairs.append(None)
                    ...
                    adv_texts.append(None)
                    adv_text_pairs.append(None)
        ...
        adversarial_info = pd.DataFrame(list(zip(texts, text_pairs, labels, preds,
                                                 adv_texts, adv_text_pairs, adv_preds, iters)),
    ...
    def random_one_attack(self, text, sent1_token_len):
        """
        randomly choose one character-level attack method to attack the text
    ...
    def add_one_attack(self, tokens, sent1_token_len, alphabetes='abcdefghijklmnopqrstuvwxyz'):
        """
        internally inserting a new character in a randomly chosen word
        ...
        :return: a list of 10 randomly chosen examples after attack or original tokens if 1000 times attacks failed
        """
    ...
    def key_one_attack(self, tokens, sent1_token_len):
        """
        substituting an internal character with one of its adjacent character in keyboards
        :param tokens: tokens of an example used for attack
        :param sent1_token_len: the length of tokens for the first sentences in tokens
        :return: examples after attack or original tokens if 1000 times attacks failed
        """
        for _ in range(1000):
            idx = random.choice(range(len(tokens)))
            token = tokens[idx]
...
class SynonymsReplacement:
    ...
    def __init__(self, predict, data_name, max_seq_length, num_labels, max_vocab_size=20000, pop_size=20, temp=0.3):
        """
        ...
        self.max_iters = int(max_seq_length // 5)   # maximum number of attacks
        self.temp = temp
    ...
    def load_neighbours_info(self, path):
        """load the neighbour information"""
        try:
    ...
    def generate(self, texts, text_pairs, labels):
        ...
        for i, (sent1, sent2, label) in enumerate(zip(texts, text_pairs, labels)):
            print(i, sent1.encode('utf8'), label)  # for testing

            if sent1 in self.neighbours_info.values:
                # example = self.neighbours_info[self.neighbours_info.values == sent1].iloc[0]
                if sent2:
                    example = self.neighbours_info.loc[
                        (self.neighbours_info['text'] == sent1) & (self.neighbours_info['text_pair'] == sent2)].iloc[0]
                else:
                    example = self.neighbours_info.loc[self.neighbours_info['text'] == sent1].iloc[0]
...
class SEAs:
    """
    This class is adapted from the code of the paper: Semantically Equivalent Adversarial Rules for Debugging NLP
    Models. We only change the target model and dataset, and keep using OpenNMT-py==0.1 because we use the translation
    models that Marco proposed. Paper Link: https://www.aclweb.org/anthology/P18-1079/
    """

    def __init__(self, predict, num_labels, topk=100, threshold=-10):
        """
        Create a SEAs attack instance.
        :param predict: function used for prediction, the goal of the attack is to change predictions of original inputs
        :param num_labels: number of classes in the dataset. 2 for IMDB, 3 for Mnli
        :param topk: get topk paraphrases as potential adversarial examples
        :param threshold: a threshold related to orig example's prediction
        """
        # set device
        if torch.cuda.is_available():
            ...
            gpu_id = -1
        self.ps = paraphrase_scorer.ParaphraseScorer(gpu_id=gpu_id)
    ...
    def generate(self, texts, text_pairs, labels):
        """
        generate phrase-level(SEAs) adversarial examples
        :param texts, text_pairs, labels: original texts, text_pairs, labels
        :return: original, adversarial examples and their labels and predictions
        """
        texts_flips = self.get_flips(texts)
        text_pairs_flips = self.get_flips(text_pairs) if text_pairs[0] else text_pairs
        preds, adv_texts, adv_text_pairs, adv_preds = \
            self.get_adv(texts, text_pairs, texts_flips, text_pairs_flips, labels)

        adv_info = pd.DataFrame(list(zip(texts, text_pairs, labels, preds,
                                         texts_flips, text_pairs_flips,
                                         adv_texts, adv_text_pairs, adv_preds)),
                                columns=['orig_text', 'orig_text_pair', 'orig_label', 'orig_pred',
                                         'texts_flips', 'text_pairs_flips',
                                         'adv_text', 'adv_text_pair', 'adv_pred'])
    ...
    def get_flips(self, x):
        """
        generate semantic equivalent sentences of x as potential adversaries
        :param x: origianl sentences
        :return: semantic equivalent sentences of x
        """
        flips_x = []

        for i, text in enumerate(x):
            instance_for_onmt = onmt_model.clean_text(' '.join([x.text for x in self.nlp.tokenizer(text)]),
                                                      only_upper=False)
            print(i)  # for testing
            paraphrases = self.ps.generate_paraphrases(instance_for_onmt, topk=self.topk, edit_distance_cutoff=4,
                                                       threshold=self.threshold)
            texts = self.tokenizer.clean_for_model(self.tokenizer.clean_for_humans([x[0] for x in paraphrases]))
            flips_x.append(texts)

        return flips_x

    def get_adv(self, texts, text_pairs, texts_flips, text_pairs_flips, y):
        """
        select adversarial examples from flipped texts
        :param texts: original texts
        :param text_pairs: original text pairs
        :param text_flips: flipped texts
        :param text_pairs_flips: flipped text pairs
        :param y: original labels
        :return: adversarial examples or None
        """
        preds, _ = self.predict(list(zip(texts, text_pairs)))
        right = np.where(preds == y)[0]

        adv_texts, adv_text_pairs, adv_preds = [], [], []
        for ind in range(len(y)):
            # only generate adversarial examples for examples having correct predictions
            if ind in right:
                if text_pairs_flips[ind]:
                    inputs = [(text, text_pair) for text in texts_flips[ind] for text_pair in text_pairs_flips[ind]]
                else:
                    inputs = list(zip(texts_flips[ind], [None] * len(texts_flips[ind])))
                flip_preds, flip_softmax = self.predict(inputs)

                target_label = list(range(self.num_labels))
                target_label.remove(preds[ind])

                succ_ind = np.where([pred in target_label for pred in flip_preds])[0]
                flip_probs = flip_softmax[:, target_label]

                tmp = flip_probs[succ_ind].squeeze()
                # select an example as an adversarial example which has the highest probability of target labels
                if len(succ_ind) > 1:
                    adv_ind = np.unravel_index(np.argsort(-tmp, axis=None), tmp.shape)[0][0]
                else:
                    adv_ind = None

                adv_texts.append(inputs[succ_ind[adv_ind]][0]) if adv_ind else adv_texts.append(None)
                adv_text_pairs.append(inputs[succ_ind[adv_ind]][1]) if adv_ind and text_pairs_flips[ind] \
                    else adv_text_pairs.append(None)
                adv_preds.append(flip_preds[succ_ind[adv_ind]]) if adv_ind else adv_preds.append(None)
            else:
                adv_texts.append(None)
                adv_text_pairs.append(None)
                adv_preds.append(None)

        return preds, adv_texts, adv_text_pairs, adv_preds
