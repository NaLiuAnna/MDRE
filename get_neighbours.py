...
# Vectors (and language model predictions) before words-level (synonym replacement) attack
import os
import math
import argparse
...
parser.add_argument('--dataset-name', type=str, default=None, required=True, choices=['IMDB', 'Mnli', 'SST2'],
                    help='To generate neighbours of words in the test set of which dataset for the following attack.')
...
def get_dictionary(dataset, vocab_file, lm_tokenizer):
    """
    generate a dictionary and sort by the number of occurrences of words in the dataset
    ...
    :return: a dictionary contains a list of words and their ids from the Transformer-XL tokenizer
    """
    if os.path.exists(vocab_file):
        ...
        if dataset.train_text_pair[0]:
            tmp = list(zip(dataset.train_text, dataset.train_text_pair)) + \
                  list(zip(dataset.valid_text, dataset.valid_text_pair)) + \
                  list(zip(dataset.test_text, dataset.test_text_pair))
            all_data = [sentences[0] + ' ' + sentences[1] for sentences in tmp]
        else:
            all_data = dataset.train_text + dataset.valid_text + dataset.test_text
...
def compute_distance_matrix(dictionary, distance_matrix_file, missed_embedding_file, counter_fitting_file,
    ...
    """
    get the top max_vocab_size words in the dictionary and
    ...
    :param missed_embedding_file: the path of the missed embedding file storing the words, which are in the
                                dictionary but do not have embeddings from the Counter-fitting Word Vectors,
                                they are not in the distance matrix file
    :param counter_fitting_file: the path of the Counter-fitting Word Vectors
...
def get_neighbours_info(examples, max_vocab_size, max_seq_length, vocab_file, distance_matrix_file,
    ...
    :param max_vocab_size: the maximum number of words in the distance matrix
    :param max_seq_length: the maximum sequences length of BERT model
    :param vocab_file: path of the dictionary file
    ...
    neighbours_list = []  # three-dimensional list, each element is neighbours of words in an examples
    neighbours_len_list = []  # two-dimensional list, each element is the number of neighbours of words in an example
    ...
    for (text, text_pair, label) in examples:
        ...
        neighbours_len = []  # a list of number of neighbours of words in has_neighbours
...
if __name__ == '__main__':
    ...
    vocab_file = os.path.join(output_dir, 'synonym/aux_files/vocab.vocab')
    counter_fitted_file = './data/counter-fitted-vectors.txt'
    distance_matrix_file = os.path.join(output_dir, 'synonym/aux_files/dist_counter.npy')
    embedding_file = os.path.join(output_dir, 'synonym/aux_files/embeddings_counter.npy')
    missed_embedding_file = os.path.join(output_dir, 'synonym/aux_files/missed_embeddings_counter.npy')

    lm_tokenizer = TransfoXLTokenizer.from_pretrained('transfo-xl-wt103')

    # get a dictionary and a matrix containing the distance between each word
    dictionary = get_dictionary(dataset, vocab_file, lm_tokenizer)
    ...
    neighbours_file = os.path.join(output_dir, 'synonym/aux_files/neighbours_info.csv')
    if not os.path.exists(neighbours_file):
        n_cpus = 20 if os.cpu_count() > 20 else os.cpu_count()
        print('number of cpus:', n_cpus)
        pool = Pool(n_cpus)
        step = math.ceil(len(dataset.test_y) / n_cpus)
        pool_inputs = [zip(dataset.test_text[i: i + step], dataset.test_text_pair[i: i + step],
                           dataset.test_y[i: i + step]) for i in range(0, len(dataset.test_y), step)]

        neighbours_list_queue = pool.map(
            partial(get_neighbours_info, max_vocab_size=max_vocab_size, max_seq_length=max_len,
    ...
    # testing
    # examples = zip(dataset.test_text, dataset.test_text_pair, dataset.test_y)
    # neighbours_info = get_neighbours_info(examples, max_vocab_size=max_vocab_size, max_seq_length=max_len,
    #                                       vocab_file=vocab_file, distance_matrix_file=distance_matrix_file,
    #                                       missed_embedding_file=missed_embedding_file, num_neighbours=num_neighbours)
