...
def choose_forward_translation(sentence, to_translator, back_translator, n=5):
    ...
    translations = [x[0] for x in translations if x[0]] # translated sentences
...
def largest_indices(ary, n):
    ...
    indices = np.argpartition(flat, -n)[-n:]    # indices of the largest 100 numbers
    indices = indices[np.argsort(-flat[indices])]   # indices that sorted from largest to smallest
    return np.unravel_index(indices, ary.shape)
...
class ParaphraseScorer(object):
    ...
    def build_common_vocabs(self):
        self.global_itos = []   # indices to source language words using two translators words
        self.global_stoi = {}   # source language words to indices using two translators words
        self.vocab_mappers = [] # global source language words indices map to local translator words indices
        self.back_vocab_mappers = []    # local translator words indices map to global source language words indices
        self.vocab_unks = []    # unknown words indices
        back_vocab_mappers = []
    ...
    def generate_paraphrases(self, sentence, topk=10, threshold=None, edit_distance_cutoff=None, penalize_unks=True, frequent_ngrams=None):
        ...
        not_in_sentence = np.array(
            list(set(self.global_stoi.values()).difference(
                set(list(orig_itoi.keys()) + [onmt.IO.UNK]))))  # index that not in the orig_ids
        ...
        while prev:
            ...
            for (b, mapper, back_mapper, unks, context,
                ...
                new_attns.append(attn)
                attenz = attn['std'].data[0].cpu().numpy()
                chosen = np.argmax(attenz, axis=1)  # this word translation corresponding to another word (attention)
                for r, ch in enumerate(chosen):
                    ch = mapping[ch]    # chosen is index, ch is the token
                    if ch in b.vocab().stoi:
                        # print("YOO")
                        ind = b.vocab().stoi[ch]
                        # print("prev", out[r, ind])
                ...
                unk_scores.append(out[:, onmt.IO.UNK])
                global_scores[:, mapper] += out # global_scores is out + unks
                ...
                for i, p_rep in enumerate(prev_distance_rep):
                    for idx, v in orig_itoi.items():
                        ...
                        if idx == onmt.IO.UNK:
                            continue
                        candidate = p_rep + [v]
                        distance = editdistance.eval(candidate, mapped_orig[:len(candidate)])   # edit distance
                ...
                if j == onmt.IO.UNK:
                    ...
                    for x, src, mapping, score_weight in zip(new_attns, src_examples, mappings, unk_scores[i]):
                        ...
                        for zidx, (word, score) in enumerate(zip(src, attn)):
                            ...
                            word = mapping[zidx]
                            new_unk_scores[word] += score * float(score_weight)
    ...
    def score_sentences(self, original_sentence, other_sentences, relative_to_original=False, verbose=False):
        # I think this is give original_sentence and other_sentences, give other_sentences scores
        ...
        for k, (to, back, mapper, back_mapper, unks) in enumerate(
            ...
            for s in other_sentences:
                ...
                for i, n in zip(orig_ids, orig_ids[1:]):
                    ...
                    n = int(back_mapper[n])
                    out, decStates, attn = back.advance_states(encStates, context,
                                                               decStates, [idx], [1])   # out is the value/probability of each words in back.vocab()
                    attenz = attn['std'].data[0].cpu().numpy()
